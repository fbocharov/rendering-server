...
#include "encoding/nvCPUOPSys.h"

#include "encoding/nvenc_backend.h"
#include "encoding/io_buffer_pool.h"
...
nvenc_backend::nvenc_backend(CUcontext cuda_device)
    : library_(load_library, unload_library)
{
    create_api_instance();
    open_encode_session(cuda_device);
    init_encoder(0, 0); // TODO: pass width and height
}
...
void * nvenc_backend::register_resource(NV_ENC_INPUT_RESOURCE_TYPE type, void * resource, uint32_t width, uint32_t height, uint32_t pitch)
{
    ...
    __nvenc_check(api_->nvEncRegisterResource(encoder_, &params));
...
{
    __nvenc_check(api_->nvEncUnregisterResource(encoder_, resource));
}
...
void * nvenc_backend::create_bitstream_buffer(uint32_t size)
{
    ...
    __nvenc_check(api_->nvEncCreateBitstreamBuffer(encoder_, &params));
...
void nvenc_backend::destroy_bitstream_buffer(NV_ENC_OUTPUT_PTR bitstream_buffer)
{
    if (bitstream_buffer)
    {
        __nvenc_check(api_->nvEncDestroyBitstreamBuffer(encoder_, bitstream_buffer));
    }
}
...
#endif
    __nvenc_check(api_->nvEncRegisterAsyncEvent(encoder_, &params));
...
void nvenc_backend::destroy_async_event(void * event)
{
    if (event)
    {
        ...
        __nvenc_check(api_->nvEncUnregisterAsyncEvent(encoder_, &params));
...
void * nvenc_backend::map_input_resource(void * input_resource)
{
    ...
    __nvenc_check(api_->nvEncMapInputResource(encoder_, &params));
...
void nvenc_backend::unmap_input_resource(NV_ENC_INPUT_PTR input_resource)
{
    if (input_resource)
    {
        __nvenc_check(api_->nvEncUnmapInputResource(encoder_, input_resource));
    }
}
...
void nvenc_backend::send_eos(void * event)
{
    ...
    __nvenc_check(api_->nvEncEncodePicture(encoder_, &encPicParams));
}
...
void nvenc_backend::init_encoder(size_t width, size_t height)
{
    ...
    __nvenc_check(api_->nvEncGetEncodePresetConfig(encoder_, params.encodeGUID, params.presetGUID, &preset_config));
    ...
    __nvenc_check(api_->nvEncInitializeEncoder(encoder_, &params));
}
